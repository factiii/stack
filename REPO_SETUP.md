...
```
your-repo/
├── infrastructure.yml          # Required: Infrastructure config (created by `core init`)
...
└── .github/
    └── workflows/
        ├── check-config.yml     # Generated by `core generate-workflows`
        ├── deploy-staging.yml   # Generated by `core generate-workflows`
        └── deploy-prod.yml      # Generated by `core generate-workflows`
```
...
After running `npx core init`, edit `infrastructure.yml`:
...
After running `npx core generate-workflows`, workflows are created:
...
2. Redeploy: `npx core deploy --environment staging`
...
- [ ] Deploy: `npx core deploy --environment staging`
- [ ] Verify: `npx core check-config --environment staging`
...
- Run `npx core check-config` to see current port assignments
