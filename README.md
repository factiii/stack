...
Generated by `npx factiii`. Contains detected stack configuration:
...
- **No modifications** - use `npx factiii fix` to upload

**Remote (staging/prod) - check-only:**
- SSH to servers (read-only)
- Validates deployed configurations
- Reports mismatches
- **No modifications** - use `npx factiii fix` for server fixes

### `npx factiii fix`

Fix everything that `npx factiii` checks - local, GitHub, and remote servers. Prepares everything for deployment.

```bash
npx factiii fix           # Fix all environments
npx factiii fix --dry-run # Preview changes only
```

**What it fixes:**

**Local:**
- Everything `npx factiii` does (all local configs, dependencies, files)
...
|-------------|--------|----------------|
| `STAGING_ENVS` | `.env.staging` file | Edit file → `npx factiii fix` |
| `PROD_ENVS` | `.env.prod` file | Edit file → `npx factiii fix` |
...
2. Run `npx factiii fix` to update deployed config

### "Deployment blocked"

Run `npx factiii` to see what's blocking, then `npx factiii fix` to resolve.
