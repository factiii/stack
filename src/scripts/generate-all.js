...
function generateDockerCompose(allConfigs) {
  ...
  for (const [repoName, config] of Object.entries(allConfigs)) {
    ...
    for (const [envName, envConfig] of Object.entries(config.environments)) {
      ...
      if (fs.existsSync(autoConfigPath)) {
        try {
          ...
          if (autoConfig.dockerfile) {
            dockerfile = autoConfig.dockerfile.split(' ')[0]; // Remove OVERRIDE if present
          }
        } catch (e) {}
      }
      ...
      const envFile = path.join(repoPath, `.env.${envName === 'production' ? 'prod' : envName}`);
      ...
      if (envConfig.port) {
        compose.services[serviceName].ports = [`${envConfig.port}:${envConfig.port}`];
      } else {
 ...
 */
function generateNginx(allConfigs) {
  const routes = [];
...
module.exports = { scanRepos, loadConfigs, generateDockerCompose, generateNginx };
