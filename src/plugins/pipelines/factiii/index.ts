  ...
  static canReach(stage: Stage, _config: FactiiiConfig): Reachability {
    switch (stage) {
      ...
      case 'secrets':
        ...
        // ============================================================
        // CRITICAL: Workflows must be committed for remote operations
        // ============================================================
        // Why this exists: GitHub Actions can only trigger workflows that
        // exist on the remote branch. If workflows are uncommitted, remote
        // scans/deploys will fail with "workflow not found" errors.
        // What breaks if changed: Remote scans would be triggered but fail.
        // Dependencies: Dev stage must fix workflow issues first.
        // ============================================================
        try {
      ...
      case 'prod':
        ...
        // (STAGING_SSH, PROD_SSH) are stored in GitHub Secrets.
        // What breaks if changed: Without this check, staging/prod would show
        // as reachable but fail when trying to access secrets or trigger workflows.
        // Dependencies: Enforces dev → secrets → staging/prod dependency chain.
        // ============================================================
        if (!process.env.GITHUB_TOKEN) {
          return {
            reachable: false,
            reason: 'Missing GITHUB_TOKEN (required for SSH keys in GitHub Secrets)',
          };
        }

        // Can reach via workflow (workflow will use secrets)
        return {
          reachable: true,
          via: 'workflow',
        };
