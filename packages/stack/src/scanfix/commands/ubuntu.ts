...
export const dockerCommands: PlatformCommands = {
  check: 'which docker',
  install:
    'sudo DEBIAN_FRONTEND=noninteractive apt-get update && sudo DEBIAN_FRONTEND=noninteractive apt-get install -y docker.io && sudo systemctl enable docker && sudo systemctl start docker && sudo usermod -aG docker $USER',
...
export const nodeCommands: PlatformCommands = {
  check: 'which node',
  install: 'sudo DEBIAN_FRONTEND=noninteractive apt-get update && sudo DEBIAN_FRONTEND=noninteractive apt-get install -y nodejs npm',
  manualFix: 'Install Node.js: sudo apt-get install -y nodejs npm',
};

export const gitCommands: PlatformCommands = {
  check: 'which git',
  install: 'sudo DEBIAN_FRONTEND=noninteractive apt-get update && sudo DEBIAN_FRONTEND=noninteractive apt-get install -y git',
