...
5. **Config patterns** - Required values use `EXAMPLE_` prefix. Auto-detected values use `OVERRIDE` pattern.
...
9. **scan MUST NOT modify files** - scan is read-only. If scan modifies anything, it is a bug. If no stack.yml exists, scan tells user to run `npx stack init`.
10. **fix MUST NOT touch deployment artifacts** - fix handles config files, tool installs, workflow files. fix MUST NOT touch docker-compose, nginx configs, containers, or SSL. Those are deploy's job. If broken, fix tells user to run `npx stack deploy`.
...
**Required values:** Prefix with `EXAMPLE_` (blocks deployment until replaced)
...
| Command | Behavior |
|---------|----------|
| `npx stack init` | Scan codebase, create stack.yml with EXAMPLE_ vars, vault/secrets setup |
| `npx stack` / `npx stack scan` | Read-only check — reports issues, NEVER modifies files |
| `npx stack fix` | Fix config files, install tools — NEVER touches docker/nginx/containers |
| `npx stack deploy` | Run scan, deploy if checks pass — handles docker/nginx/containers/SSL |
...
- Forgetting `EXAMPLE_` prefix on required factiii.yml values
